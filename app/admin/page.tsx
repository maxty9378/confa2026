...
export default function AdminPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([settingsData, districtsData, statsData, votesData]) => {
        setCourseTestGdf(
        ...
        setRecentVotes(Array.isArray(votesData) ? votesData : []);
      })
  ...
  const handleDeleteVote = async (id: number) => {
    if (!confirm(`Удалить ответ #${id}?`)) return;
    setDeletingVote(id);
    try {
      ...
      const data = await res.json();
      setStats(data.stats);
      setRecentVotes((prev) => prev.filter((v) => v.id !== id));
